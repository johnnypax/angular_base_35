...
export class PathController {
  constructor(private readonly service = new PathService()) {}
  ...
  create = async (req: Request<unknown, unknown, PathPayload>, res: Response, next: NextFunction): Promise<void> => {
    try { res.status(201).json(await this.service.create(req.body)); } catch (e) { next(e); }
  };

  update = async (req: Request<{ id: string }, unknown, PathPayload>, res: Response, next: NextFunction): Promise<void> => {
